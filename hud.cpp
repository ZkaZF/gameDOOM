...
static void drawArenaPanel(void) {
    ...
    for (ai = 0; ai < 2; ai++) {
        ...
        { int tw = textWidth(header, GLUT_BITMAP_HELVETICA_12); drawText(px+(bw-tw)*0.5f, py+14, header, GLUT_BITMAP_HELVETICA_12); }
        ...
        { int tw = textWidth(a->statusMsg, GLUT_BITMAP_HELVETICA_18); drawText(px+(bw-tw)*0.5f, py+40, a->statusMsg, GLUT_BITMAP_HELVETICA_18); }
...
static void drawWeaponSelector(void) {
    ...
    for (i = 0; i < NUM_WEAPONS; i++) {
        ...
        { int tw = textWidth(names[i], GLUT_BITMAP_HELVETICA_12); drawText(ix+(iW-tw)/2.0f, iY+15, names[i], GLUT_BITMAP_HELVETICA_12); }
    }
}
