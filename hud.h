...
static void drawArenaPanel(void) {
    ...
    for (ai = 0; ai < 2; ai++) {
        ...
        sprintf(header, ai == 0 ? "TOP ROOM" : "BOTTOM ROOM");
        glColor4f(0.04f, 0.04f, 0.08f, 0.82f);
        fillRect(px - 2, py - 2, bw + 4, bh + 4);
        if (a->state == ARENA_COMPLETE)
            glColor4f(0.20f, 0.90f, 0.30f, 0.80f);
        else if (a->state == ARENA_COOLDOWN)
            glColor4f(0.90f, 0.70f, 0.10f, 0.80f);
        else
            glColor4f(0.90f, 0.15f, 0.10f, 0.80f);
        strokeRect(px - 2, py - 2, bw + 4, bh + 4);
        glColor3f(0.70f, 0.70f, 0.75f);
        {
            int tw = textWidth(header, GLUT_BITMAP_HELVETICA_12);
            drawText(px + (bw - tw) * 0.5f, py + 14, header, GLUT_BITMAP_HELVETICA_12);
        }
        if (a->state == ARENA_COMPLETE)
        ...
        {
            int tw = textWidth(a->statusMsg, GLUT_BITMAP_HELVETICA_18);
            drawText(px + (bw - tw) * 0.5f, py + 40, a->statusMsg, GLUT_BITMAP_HELVETICA_18);
        }
        if (a->state == ARENA_ACTIVE || a->state == ARENA_COOLDOWN) {
            ...
            glColor4f(0.15f, 0.15f, 0.18f, 0.80f);
            fillRect(barX, barY, barW, barH);
            glColor3f(0.90f, 0.30f, 0.10f);
